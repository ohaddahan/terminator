...
impl<Tab> DockArea<'_, Tab> {
    pub(super) fn show_leaf(
        ...
        let leaf = self
            ...
            .leaf(path)
            .expect("This node must be a leaf");
        let policy_hidden = !tab_viewer.show_tab_bar(path);
        let tab_bar_hidden = policy_hidden || self.hidable_tab_bars && leaf.tab_bar_hidden;
        ...
        } else if policy_hidden {
            Rect::from_min_size(ui.cursor().min,vec2(ui.available_width(),0.0))
        } else {
        ...
        if tab_bar_hidden && !policy_hidden {
    ...
    fn tab_bar(
        ...
        if self.show_add_buttons {
            available_width -= Style::TAB_ADD_BUTTON_SIZE + tab_viewer.trailing_controls_width();
        }
        ...
        let (actual_width, tab_hovered) = {
            ...
            if self.show_add_buttons {
                ...
                let trailing = tab_viewer.trailing_controls_width();
                self.tab_plus(ui, path, tab_viewer, tabbar_outer_rect, offset + trailing, fade_style);
                if trailing > 0.0 {
                    let rect = Rect::from_min_size(pos2(tabbar_outer_rect.right() - offset - trailing, tabbar_outer_rect.top()), vec2(trailing, tabbar_outer_rect.height()));
                    let mut controls = ui.new_child(UiBuilder::new().max_rect(rect).layout(Layout::left_to_right(Align::Center)).id_salt(("trailing", path)));
                    tab_viewer.trailing_controls(&mut controls, path);
                }
            }
    ...
    fn tab_plus(
        ...
        response = response.on_hover_cursor(CursorIcon::PointingHand).on_hover_text("New tab in this pane");
